defmodule MingleMeWeb.UserAuthTest do
  use MingleMeWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias MingleMe.Accounts
  alias MingleMe.Accounts.Scope
  alias MingleMeWeb.UserAuth

  import MingleMe.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MingleMeWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      MingleMeWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        MingleMeWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
